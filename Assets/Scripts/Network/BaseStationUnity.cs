...
public class BaseStationUnity : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        // Get the Logistic Center script
        logisticCenterUnityScript = FindObjectOfType<LogisticCenterWithMininetWifi>();
    ...
    void WaitForDrones()
    {
        ...
        if(logisticCenterUnityScript.totalNumberOfDrones == messageReceivedByDrones.Count)
        {
            ...
            logisticCenterUnityScript.firstMessageFlag = false;
    ...
    void UnityToUnityReceiveMessageFromDrone()
    {
        ...
        for (int i = 0; i < countMessagesPerFrame; i++)
        {
            ...
            logisticCenterUnityScript.messageBaseStationToLogisticCenter.Add(decodeDroneInfo.playerName,messagesToProcess[i]);
